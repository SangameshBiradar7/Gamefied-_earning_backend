...
router.get('/dashboard', auth, adminAuth, async (req, res) => {
  try {
    ...
    const allStudents = await User.find({ role: 'student' })
      .select('points level streak completedLessons weeklyTestResults grade')
    ...
    // Top streaks
    const topStreaks = allStudents
      .sort((a, b) => (b.streak?.current || 0) - (a.streak?.current || 0))
      .slice(0, 10)
      .map(s => ({
        _id: s._id,
        username: s.username,
        streak: s.streak || { current: 0, longest: 0 },
        grade: s.grade
      }));

    // Most active by completed lessons
    const mostActive = allStudents
    ...
    const gradePerformance = await Promise.all(grades.map(async (grade) => {
      const gradeStudents = allStudents.filter(s => s.grade && s.grade.toString() === grade._id.toString());
      const totalStudents = gradeStudents.length;
      const avgPoints = totalStudents > 0
        ? Math.round(gradeStudents.reduce((sum, s) => sum + (s.points || 0), 0) / totalStudents)
        : 0;
      const avgStreak = totalStudents > 0
        ? Math.round(gradeStudents.reduce((sum, s) => sum + (s.streak?.current || 0), 0) / totalStudents * 10) / 10
        : 0;
      const totalCompletedLessons = gradeStudents.reduce((sum, s) => sum + (s.completedLessons?.length || 0), 0);
    ...
    const inactiveStudents = allStudents.filter(s => {
      const lastActive = s.completedLessons?.length > 0
        ? s.completedLessons[s.completedLessons.length - 1]?.completedAt
        : null;
      return !lastActive || (new Date() - new Date(lastActive)) > 7 * 24 * 60 * 60 * 1000;
    }).slice(0, 5).map(s => ({ username: s.username, grade: s.grade }));

    const lowPerformers = allStudents
      .filter(s => (s.points || 0) < 200)
    ...
    const lostStreaks = allStudents
      .filter(s => s.streak && s.streak.current === 0 && s.streak.longest > 0)
      .sort((a, b) => (b.streak.longest || 0) - (a.streak.longest || 0))
      .slice(0, 5)
      .map(s => ({
        username: s.username,
        streak: s.streak,
    ...
    // Calculate totals
    const totalXP = allStudents.reduce((sum, s) => sum + (s.points || 0), 0);
    ...
    const lessonsToday = allStudents.reduce((sum, s) => {
      return sum + (s.completedLessons?.filter(l =>
        l.completedAt && new Date(l.completedAt) >= startOfDay
      ).length || 0);
    }, 0);
    ...
    for (let i = 13; i >= 0; i--) {
      ...
      const activeCount = allStudents.filter(s =>
        s.completedLessons?.some(l =>
          l.completedAt && new Date(l.completedAt) >= dayStart && new Date(l.completedAt) < dayEnd
        )
      ).length;
      ...
      dailyActivity.push({
        ...
        count: activeCount,
        uniqueStudents: activeCount
      });
    }

    // Village analytics (if village field exists)
    const villages = await User.distinct('village', { role: 'student', village: { $ne: null, $ne: '' } });
    const villageAnalytics = await Promise.all(villages.map(async (village) => {
      const villageStudents = allStudents.filter(s => s.village === village);
      return {
        village,
        totalStudents: villageStudents.length,
        avgPoints: Math.round(villageStudents.reduce((sum, s) => sum + (s.points || 0), 0) / villageStudents.length) || 0,
        avgStreak: Math.round(villageStudents.reduce((sum, s) => sum + (s.streak?.current || 0), 0) / villageStudents.length * 10) / 10 || 0
      };
    }));
    ...
    res.json({
      totalStudents,
      activeStudents,
      averagePoints: Math.round(totalXP / totalStudents) || 0,
      totalPoints: totalXP,
      perfectStreaks: allStudents.filter(s => (s.streak?.current || 0) >= 7).length,
      lessonsToday,
      newToday,
      topStudents,
      topStreaks,
    ...
    // Get all students for analytics
    const allStudents = await User.find({ role: 'student' }).select('points level streak completedLessons weeklyTestResults');
